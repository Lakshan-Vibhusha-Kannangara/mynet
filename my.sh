...
if command -v docker >/dev/null 2>&1; then
    ...
    apt update --update-cache --repository http://dl-cdn.alpinelinux.org/alpine/v3.14/main
    apt install --no-cache ca-certificates curl gnupg
    mkdir -m 0755 -p /etc/apk/keys
...
if [ ! -z "$NVIDIA_PRESENT" ]; then
    if docker run --gpus all nvidia/cuda:11.0.3-base-ubuntu18.04 nvidia-smi >/dev/null 2>&1; then
        ...
        distribution=$(cat /etc/os-release | grep '^ID=' | cut -d= -f2 | tr -d '"')$(cat /etc/os-release | grep '^VERSION_ID=' | cut -d= -f2 | tr -d '"')
        curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey | apt add --update-cache --repository http://dl-cdn.alpinelinux.org/alpine/v3.14/main --no-cache gnupg
        curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list | tee /etc/apk/repositories
        apt --no-cache update --update-cache --repository http://dl-cdn.alpinelinux.org/alpine/v3.14/main
        apt --no-cache -y install nvidia-container-toolkit
...
apt add --no-cache nvidia* libnvidia*
